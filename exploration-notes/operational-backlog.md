...
16. **Outbound Xero export tool — real-client-export validation OPEN (built on branch `t-xero-outbound-export`, 2026-07-06, UNMERGED).**
   ...
   - **Open sub-items for Terry:** (i) reconcile the Xero CSV **column spec** in `exports/xero_export.py` (`INVOICE_COLUMNS`/`BILL_COLUMNS`/`CONTACT_COLUMNS`) against Collin's actual Xero import template files (required sets taken as given; optional columns not fabricated); (ii) Bills `InvoiceNumber` uses internal `DocNum` because `NumAtCard` (supplier ref) is null across the SBODEMOSG capture — confirm; (iii) crosswalk carries 9 Annex E targets (DS/ESN33/EP/OP/IGDS/ME/TX-ESS/TX-N33/TX-RE) not exercised by demo data — untested against real transactions.

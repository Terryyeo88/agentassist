...
17. **Stale-local-master trap — start-of-build guard ADDED (Option 1, script-only; branch `t-preflight-base-check`, 2026-07-08).**
   ...
   - **Scope — start-stale ONLY; mid-flight-stale NOT covered.** This guard catches a base that is stale **at the moment work starts**. A base that goes stale **DURING** a build (someone else merges to master mid-task) is a **separate, still-open problem** — this item does NOT close it. Do not read item 17 as "stale-base fully solved."

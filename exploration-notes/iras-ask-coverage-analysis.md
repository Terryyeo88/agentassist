...
| 3D.1.1.h — Purchases from non-GST-registered suppliers | Supplier GST-registration status | ✓ | NO_GST_REG (`detect_gst_errors`): purchase with input tax + blank `FederalTaxID` | Relies on `FederalTaxID`; clients storing GST reg no. in a UDF would mis-flag (config). **Xero:** the Xero readers' coverage POPULATION is now OBSERVED from the data, not asserted by fiat (`D-2026-07-27-xero-coverage-derived`) — but `FederalTaxID` stays outside the covered set, so **NO_GST_REG remains UNAVAILABLE on Xero** (reason byte-identical) pending the supplier-master surface (PR-2, gated on Terry rulings D-3..D-8). The coverage cell above is the SAP/extract position and is unchanged. **CITATION (not coverage) corrected 2026-07-28 (`D-2026-07-28-nogstreg-recite`, ruling D-24):** the registry `iras_basis` for this check moved from `IRAS GST Act s19(1) / Regulation 11` to `GST (General) Regulations [2026 Ed.], reg 11 — Conditions for claiming input tax`, because at that time `sources.md:64` recorded the GST Act as ABSENT/UNVERIFIED while `sources.md:65` recorded the Regulations as CURRENT and sha256-pinned. **(SUPERSEDED same day: the rule-author supplied the Act — `gst-act-1993-2020ed.pdf`, `[2020 Ed.]`, 319 pp, sha256 `30bddc8f…` — and `sources.md:64` is now CURRENT. `s19(1)` is NOT auto-restored; D-24 stands as a substantive authoring decision, reg 11 being arguably the more precise authority for *conditions for claiming*. Coverage cell still ✓, still not re-argued.)** **The ✓ cell is UNCHANGED and was NOT re-argued** — detection is byte-for-byte the same check over the same field; only the citation shown beside a finding changed. The citation is itself UNVALIDATED (the shipped `iras_basis_caveat` says so). **The RECOMMENDATION text (`sap_b1_server.py:1514`) is NOT corrected** — that is D-25 and requires an oracle re-freeze. **RECOMMENDATION text corrected 2026-07-28 (`D-2026-07-28-nogstreg-reword`, rulings D-25/D-26):** `sap_b1_server.py:1514` no longer ends "or reverse the input tax claim" — an instruction to alter the client's books that presupposed invalidity, breaching surfaces-never-asserts. It now hands the determination to the reviewer. **This RE-FROZE the offline-replay oracle** (59fbdbb8… → 0225df39…) under D-22, proven field-by-field: 7 differing paths, all 7 the recommendation, 0 added, 0 removed. **The ✓ cell is UNCHANGED and was NOT re-argued** — same documents, same count, same severity, same description; only the sentence a reviewer reads changed. **P4 is now COMPLETE** (D-24 citation + D-26 recommendation). |
...
| 3D.1.1.h Purchase from non-GST-registered supplier (→ NO_GST_REG) | D+ | Deterministic from `FederalTaxID`; the "+" is config for clients storing reg no. in a UDF. **Xero:** coverage population is now observed, not by-fiat (`D-2026-07-27-xero-coverage-derived`); NO_GST_REG remains unavailable-on-Xero pending the supplier-master surface (PR-2, Terry rulings D-3..D-8) — the class above is unchanged |
...
> **Mechanism note (T2.28, built ≠ validated).** `reasoning/sap_lines.py` gains **`fetch_sales_lines`** — the SALES analog of the purchase-side `fetch_si_purchase_lines` — a **passive line source** that surfaces ALL sales lines (Invoices + CreditNotes) each carrying its own **canonical** VatGroup (ES33 / SO / OS / ZR on SBODEMOSG, where normalization is an identity passthrough), with **no VatGroup filter** so a downstream `SkillSpec` selects the codes it needs. It is the data-plumbing a future **sales-side** reasoning skill (e.g. an exempt-supply / Reg 33 classification skill) would consume. It changes **nothing** about coverage or validation: **no coverage cell (◐/●) moves**, no VatGroup→F5-box routing changes, no tax semantics or candidate or skill is authored, and it is **not wired** into `run_chain`, `calculate_f5_return`, any gate/E-check, or any reasoning pass (F5 boxes / gates / offline-replay oracle byte-identical). SAP path only — the Xero sales path stays STOP-FLAGGED. Nothing here is claimable; **T2.11 still gates every reasoning-layer accuracy claim.**

> **Mechanism note (T2.29, built ≠ validated).** `SkillSpec.vat_group` (in `reasoning/reasoning_pass.py`) is widened from a single code (`str`) to **`str | frozenset[str]`**, so one reasoning skill can select a **SET** of canonical tax codes (e.g. `frozenset({"ES33","ESN33"})`) — the selection filter becomes membership over `allowed_vat_groups()` and a multi-code candidate keeps its OWN per-line code rather than being flattened to one value. This is the typing/branching half of the "exempt-supply skill" plumbing (the passive sales line source is T2.28) — it lets a future sales-side skill pick the exempt codes it cares about. It changes **nothing** about coverage or validation: **no coverage cell (◐/●) moves**, no VatGroup→F5-box routing changes, no tax semantics / skill / KB slice is authored. **reg2627 (a single-code `str` spec) is byte-identical** — output + provenance + bundle-key unchanged, its locked characterization tests green — and the frozenset path is exercised **only by a TEST-ONLY synthetic spec**; no production reasoning stream uses a frozenset spec, and `engine/review.py` still runs only reg2627. Nothing here is claimable; **T2.11 still gates every reasoning-layer accuracy claim.**

> **Mechanism note (T2.30, built ≠ validated).** `reasoning/exempt.py` adds the **SECOND production reasoning skill** on the T2.27 shell — **`EXEMPT_SPEC`** (`skill_id="exempt-supply"`, check `exempt-supply-misclassification`, `prompt_version="t2.30-exempt-supply-v1"`) — the exempt-supply misclassification pass on the sales side. It selects lines coded **`frozenset({"ES33","ESN33"})`** (the T2.29 set filter; both land in F5 **Box 3**), draws them from **`fetch_sales_lines`** (the T2.28 passive source), and surfaces candidates against a 9-slug `suspected_categories` enum (`intermediary_fee`, `commercial_property`, `real_estate_agent`, `movable_furniture`, `mixed_use_property`, `overseas_financial_service`, `dpt_intermediary`, `not_fourth_schedule`, `indeterminate`). **ALL tax semantics live in the committed, cross-verified `knowledge-base/slices/exempt-supply.md`** (SHA-256 `14b6e5f3d039d746b1603ee44042e8b9713c2f4a2f457cb6eac3c9f11cacd7b3`, stamped into `provenance.kb_slice_hash`) — the module authors WIRING only. It is now **wired into `review()`** (a new Phase 2b beside the deterministic chain, `ReviewInputs.sales_line_source` + `ReviewResult.exempt_artefact`, riding the T2.27 seal/report seams), which closes T2.27's still-pending second-stream item. It changes **nothing** about coverage or validation: **no coverage cell (◐/●) moves**, no VatGroup→F5-box routing changes, boxes/gates/offline-replay oracle byte-identical, reg2627 byte-identical. A **live mechanism DEMO** (`claude-sonnet-4-6` over the frozen SBODEMOSG ES33 lines + one crafted `"OFFICE UNIT LEASE Q3"` trap, raw output saved before inspection) shows the wire is live and every `phrasing` begins "Consider reviewing whether" — but this is **mechanism/demo-validated on SAP-frozen, NOT accuracy-validated: no ground-truth labels exist or are claimed**. **ESN33 is absent from the frozen extract** (ES33 only; ESN33 is synthetic/hermetic only); the **Xero exempt path stays STOP-FLAGGED**; the exempt validation SET (the T2.13 analog, independently labelled) is a SEPARATE later build — this build authors no fixture labels. `validation_status="unvalidated"` / `show_ai_candidates=False` FROZEN. Nothing here is claimable; **T2.11 still gates every reasoning-layer accuracy claim.**

> **Mechanism note (T-xero-desc, built ≠ validated).** `feeders/xero_sales_reader.py` now emits **`line_description`** (a `str`) on **every** sales chain-line dict — the Xero export's per-line Description column **passed through verbatim** under the **exact reasoning-contract key name** so the future Prompt-E adapter is a straight passthrough. **Tolerant** (blank cell or missing column → `""`, never crashes) and **append-only** (`VatGroup` / `LineTotal` / `TaxTotal` untouched). **Scope ruling (Terry, binding): description emit ONLY** — no adapter, no `sales_line_source` wiring, no `reasoning/` change; the `doc_num` int-vs-str mismatch (**STOP#2**, Xero's non-numeric `InvoiceNumber` vs the numeric `doc_num` the reasoning-line contract expects) is **DEFERRED to Prompt E**. It changes **nothing** about coverage or validation: **no coverage cell (◐/●) moves**, no VatGroup→F5-box routing changes; **box-isolation is proven at runtime** — `calculate_f5_return` over the real reader vs a stripping wrapper is **byte-identical**. **Prompt E (NOT yet built) is the piece that makes this live** — the chain-doc → reasoning-line adapter + `sales_line_source` wiring in the Xero upload path + the STOP#2 `doc_num` ruling — so until then the field is **inert by design** and the exempt skill (T2.30) does **NOT** run on Xero yet. **Honest status (verbatim):** line_description emit is real-FORMAT over synthetic Xero, NOT real-client-validated; field is INERT until the Prompt E adapter wires sales_line_source — the exempt skill does NOT run on Xero yet. Nothing here is claimable; **T2.11 still gates every reasoning-layer accuracy claim.**

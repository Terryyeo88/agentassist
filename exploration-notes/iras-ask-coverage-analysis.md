...
| 3D.1.1.h — Purchases from non-GST-registered suppliers | Supplier GST-registration status | ✓ | NO_GST_REG (`detect_gst_errors`): purchase with input tax + blank `FederalTaxID` | Relies on `FederalTaxID`; clients storing GST reg no. in a UDF would mis-flag (config). **Xero:** the Xero readers' coverage POPULATION is now OBSERVED from the data, not asserted by fiat (`D-2026-07-27-xero-coverage-derived`) — but `FederalTaxID` stays outside the covered set, so **NO_GST_REG remains UNAVAILABLE on Xero** (reason byte-identical) pending the supplier-master surface (PR-2, gated on Terry rulings D-3..D-8). The coverage cell above is the SAP/extract position and is unchanged. **CITATION (not coverage) corrected 2026-07-28 (`D-2026-07-28-nogstreg-recite`, ruling D-24):** the registry `iras_basis` for this check moved from `IRAS GST Act s19(1) / Regulation 11` to `GST (General) Regulations [2026 Ed.], reg 11 — Conditions for claiming input tax`, because `sources.md:64` records the GST Act as ABSENT/UNVERIFIED while `sources.md:65` records the Regulations as CURRENT and sha256-pinned. **The ✓ cell is UNCHANGED and was NOT re-argued** — detection is byte-for-byte the same check over the same field; only the citation shown beside a finding changed. The citation is itself UNVALIDATED (the shipped `iras_basis_caveat` says so). **The RECOMMENDATION text (`sap_b1_server.py:1514`) is NOT corrected** — that is D-25 and requires an oracle re-freeze. |

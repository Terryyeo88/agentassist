...
Tier registry; PreToolUse justification gate; PostToolUse audit logger; append-only hash-chained justification ledger (same construction as T1.5 bundle; sealed into the bundle; signed PDF gains an "agent activity appendix"); deterministic Tier-2 executor; adversarial test suite (skip-justification → blocked; Tier-3 tool request → tool-not-found; Tier-2 self-execution attempt → impossible by construction). Pure Python + Agent SDK plumbing; buildable against fixtures, no live SAP. Effort ~1–2 wk. Owner: Terry. HARD ORDERING: T5.2 before T5.3 — retrofitting safety onto a working agent is how bounded autonomy quietly becomes unbounded. Also defines the check-registry schema (check ID, ASK paragraph, inputs needed, finding schema) — the coordination contract with the deterministic-check track (Collin). Runtime guards: a hard max-turn cap and per-run token/cost metering with a runaway-loop kill; cost-per-run is written into the justification ledger so cost-per-review is auditable and priceable (per-engagement pricing depends on it), and an agent that hits its cap fails into the non-blocking path (invariant 7), never an unbounded run. Tier-2 proposal contract + approval mechanism: define the proposal-artifact schema (action, tier, justification, evidence refs, inputs-hash) and a minimal approval mechanism (CLI-first: list pending proposals, show justification + evidence, approve/reject by ID) so a proposal is always reviewable, never emitted into a void — rich UI deferred, the mechanism is not.

...
### T5.7 — Agent-behaviour eval harness — SPLIT: T5.7a DONE (2026-06-14), T5.7b REMAINING
Operationalises cross-cutting requirement (a): the scenario harness that makes "no entry advances past built without evals" executable. This is offline test/measurement infrastructure (analogous to the reasoning layer's measurement.py), NOT a production agent workflow. Split into the cage-invariant basket (measurable before the loop existed — now DONE) and the loop-quality basket (needs the loop — now unblocked).
...
**T5.7b — loop-quality metrics — REMAINING (now unblocked since the Slice-2 loop exists).** The two deferred basket metrics: dossier completeness rate and language-lint pass rate, plus the prompt-injection fixtures from cross-cutting (b) wired as a scored category. These need the gather→act→verify loop to measure (it now exists). Gating dependency for any "built" → "validated" graduation of T5.3–T5.5 — it does NOT follow T5.6 despite its number. Effort ~1 wk.

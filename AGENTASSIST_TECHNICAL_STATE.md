...
**Honest status.** Built + hermetically tested + **API-verified over HTTP on synthetic real-format data** (`:8010`, all three pairings and the 422 suffix gate) + **BROWSER-VERIFIED: Terry completed the manual acceptance above on 2026-09-23 and it PASSED.** The pairing-1 script ran as written — one `NO_GST_REG` row (BILL-3003 / NoReg Trading) with live adjudication controls, the coverage row `degraded` at 19 of 20 naming the one nameless line, the signed paper carrying the Xero wording and no Section 6 supplier-registration line, and the empty-Contacts re-run returning to `unavailable`. **NOT real-client-validated** — no real Xero export has ever been read (DEBT-3). **NOT accuracy-validated**: this slice makes a check RUNNABLE; whether its findings are correct is T2.11's question and **T2.11 is unmoved**. A finding here is a CANDIDATE for a reviewer, never a verdict, and the check tests only whether a TaxNumber is PRESENT. `validation_status` unvalidated; `show_ai_candidates` False. Still open and untouched: **G-5** (#49), **G-7**, **G-8**.

---

## Slice D — unmapped tax codes must never silently reduce a box (`D-2026-09-21-unmapped-codes`)

Updated 2026-09-21 (branch `t-slice-d-unmapped-codes`, base `82a7bf9`, **MERGED via PR #182** on 2026-09-23; id still **PROPOSED** — Terry ratifies once his manual acceptance completes, so merged is not ratified here). **Open item #49 CLOSED. G-5 CLOSED.** Two defects, one root cause.
...
**OPEN ITEM FILED, NOT BUILT — THE APPEND-ONLY TEST GUARD IS BYPASSABLE BY CONSTRUCTION.** *(number to be assigned by Terry — open-item numbers are his, never grep-maxed.)* The "never edit an existing test" boundary is enforced by a PreToolUse hook (`.claude/hooks/block_existing_test_edits.py`, registered in `.claude/settings.json`) that intercepts **Edit / Write / MultiEdit / NotebookEdit** on any existing file under `tests/`. It does **not** intercept **Bash**. An agent that writes to the same file with `sed`, a heredoc or a Python one-liner is not blocked, not warned, and leaves no trace that a guard was meant to apply. **Demonstrated in this build, under explicit authorisation:** the 17 Slice-D test amendments were applied through the shell and the hook never fired.

**Why CI is not the backstop people assume.** `CLAUDE.md` already notes that CI re-running the full suite catches a test *removed* out-of-band. It does not catch a test *weakened* out-of-band: an assertion loosened via the shell still passes, so a green suite is consistent with the guard having been silently defeated. Deletion is loud; weakening is silent, and weakening is the failure mode the boundary exists to prevent.

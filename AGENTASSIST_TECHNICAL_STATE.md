...
**Open items filed by this build (#45–#46), NONE resolved. (#43 stays RESERVED-unfiled — the rule-author's soft-reserved signable-render guard; this build does NOT touch it.)**
...
**Smaller notes for the rule-author.** Reviewer identity at decision time is a **`WEB_REVIEWER` constant** (`"web-demo-reviewer"`) — capture a real pre-sign reviewer identity later. The **merged frozen+persisted `/review` view is LOOKUP-ONLY** and must never be `verify()`-ed — each store verifies as its own independently-rooted chain. **Ledger-recon rows are not fingerprinted** (different finding shape).
